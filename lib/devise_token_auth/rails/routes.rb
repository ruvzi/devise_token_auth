...
module ActionDispatch::Routing
  class Mapper
    def mount_devise_token_auth_for(resource, opts)
      ...
      route                  = opts[:as] || resource.pluralize.underscore.gsub('/', '_').to_sym
      ...
      unless rake_task_db_or_assets?
        ...
        unnest_namespace do
          ...
          devise_scope mapping_name.to_sym do
            ...
            get "#{full_path}/validate_token", controller: token_validations_ctrl.to_s, action: 'validate_token', defaults: {format: :json} if !opts[:skip].include?(:token_validations)
            ...
            if defined?(::OmniAuth) && !opts[:skip].include?(:omniauth_callbacks)
              ...
              match "#{full_path}/:provider/callback",  controller: omniauth_ctrl, action: 'omniauth_success', via: [:get], as: :omniauth_callback
